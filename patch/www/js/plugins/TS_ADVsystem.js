...
ADV_System.prototype.viewMesAdjust = function(text) {
	...
	if(ConfigManager.langSelect == 1){
		// [한글패치] 원본은 창 폭 보정용으로 전각 5~7칸을 들여쓴다(SPACE_AJASUTO).
		// 한글은 글자 폭이 커서 같은 칸수로도 왼쪽 여백이 과해 보이므로 1~3칸으로 줄인다.
		// 들여쓰기 칸도 줄당 글자 수에 포함되므로, 줄어든 만큼 본문 쪽이 넓어진다.
		text_f_space   = "　";
		text_space     = "　　";
		text_space_one = "　　　";
	}
	...
	if(ConfigManager.langSelect == 1){
		// [한글패치] En 슬롯은 한국어다. 한글은 전각 폭이라 영어용 2배(27*2=54자)를 쓰면
		// 메시지 창 폭(1024px - 패딩 36px = 988px)을 넘어 오른쪽이 잘린다.
		// 위에서 들여쓰기를 1~3칸으로 줄인 만큼 본문을 넓게 쓸 수 있어 43자로 잡는다.
		// 전체 대사 실측 최대 961px / 창 988px — 여유가 27px뿐이므로 텍스트를 고칠 때마다
		// tools/check_line_width.py로 확인할 것. 44자면 4행이 안전선을 넘는다.
		view_text_num = 43;
	}
